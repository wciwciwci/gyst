...
class PL_UI_Site {
  ...
  function pl_link_credit() {
    ...
    if ( ! pl_user_setting( 'hide_pl_cred' ) || ! pl_is_professional() ) {
      printf( '<a class="pl-link-credit" href="https://www.pagelines.com?r=plf" title="Built using PageLines" style="display: block; visibility: visible;"><i class="pl-icon pl-icon-pagelines"></i> <span class="txt">PageLines</span></a>' );
    }
  ...
  function pl_body_classes( $wp_classes ) {
    ...
    if ( ! is_user_logged_in() ) {
      ...
      $wp_classes[] = 'logged-in';
    }

    $wp_classes[] = 'pl-platform';

    $wp_classes[] = ( pl_is_professional() ) ? 'plv-pro' : 'plv-free';

    // ensure no duplicates or empties
    $wp_classes = apply_filters( 'pl_body_classes', array_unique( array_filter( $wp_classes ) ) );
