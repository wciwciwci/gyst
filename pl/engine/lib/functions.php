...
function pl_get_terms_for_selection( $post_type = false ) {
  ...
  foreach ( $taxonomies as $slug => $data ) {
    ...
    $joint = sprintf( '%s__plallterms', $slug );

    $opts[ $joint ] = array(
      'name'  => sprintf( '%s (All)', $data->labels->name ),
    );
...
/**
 * pre_get_posts filter for select_terms
 * If the tax_query terms is set to 'plallterms' then modify the tax_query to get all terms
 */
add_filter( 'pre_get_posts', 'select_terms_filter' );
function select_terms_filter( $query ) {

  if ( isset( $query->query_vars['tax_query'] ) && isset( $query->query_vars['tax_query'][0]['terms'][0] ) ) {
    if ( 'plallterms' == $query->query_vars['tax_query'][0]['terms'][0] ) {
      unset( $query->query_vars['tax_query'][0]['terms'] );
      unset( $query->query_vars['tax_query'][0]['field'] );
      $query->query_vars['tax_query'][0]['operator'] = 'EXISTS';
    }
  }
...
function pl_button_link_options( $key, $defaults = array() ) {
    ...
    foreach ( $opts as &$o ) {
      ...
      if ( isset( $defaults['priv'] ) ) {
        $o['priv'] = $defaults['priv'];
      }
    }

    unset( $o ); // set by reference

    return apply_filters( 'pl_button_link_options', $opts, $key, $defaults );
}
...
function pl_is_professional() {
  ...
  if ( true === PL_Platform()->oauth->is_local_and_has_pro() ) {
    return true;
  } else {
...
if ( ! function_exists( 'pl_btn_classes' ) ) {
  ...
  function pl_btn_classes() {
    ...
    return apply_filters( 'pl_btn_classes', $array );
  }
}
...
if ( ! function_exists( 'pl_button_sizes' ) ) {
  ...
  function pl_button_sizes() {
    ...
    return apply_filters( 'pl_button_sizes', $array );
  }
}
...
function pl_disabled_region( $region ) {

  if ( pl_can_use_tools() && pl_is_workarea_iframe() ) {

    $text = apply_filters( 'pl_disabled_region_text', sprintf( '<strong>%s</strong> %s',
        __( 'Admin Notice:', 'pl-platform' ),
        sprintf( __( 'The %s region has been disabled.', 'pl-platform' ), $region )
    ), $region );

    echo apply_filters( 'pl_disabled_section', sprintf( '<div class="pl-col-sm-12"><div class="pl-alert pl-alert-info">%s</div></div>',$text ), $region );
  }
}

function pl_is_region_disabled( $region ) {

  $post_id = pl_current_page_id();

  $setting = "_pl_$region";

  $disabled = false;

  if ( get_post_meta( $post_id, $setting, true ) ) {
    $disabled = true;
  }
  return apply_filters( 'pl_disabled_region', $disabled, $region, $post_id );
}
...
/**
 * pl_maybe_unserialize, uses a preg_replace_callback to fix broken serialized strings
 *
 * @var string serialized data in
 *
...
function pl_maybe_unserialize( $data ) {
    ...
    if ( ! is_array( $result ) ) {
      ...
      $data = preg_replace_callback( '!s:(\d+):"(.*?)";!', 'pl_maybe_unserialize_callback', $data );
...
function pl_maybe_unserialize_callback( $matches ) {
 ...
 * @return json string
 */
function pl_wp_json_encode( $object_or_array ) {
...
function pl_json_prep_data( $data ) {
  ...
  if ( is_array( $data ) || is_object( $data ) ) {
    $data = pl_wp_json_encode( $data ); } elseif ( is_int( $data ) ) {
...
function pl_get_url() {
  ...
  return 'https://www.pagelines.com/';
}
...
function pl_enc( $object_or_array, $quotes = false ) {
  $encode = wp_json_encode( pl_convert_arrays_to_objects( $object_or_array ) );
...
function pl_uploads_shortcode_url( $full_url ) {
  ...
  return apply_filters( 'pl_uploads_url', $url );
}
...
function pl_is_local() {
    ...
    $ip_loc    = (bool) filter_var( $domain, FILTER_VALIDATE_IP ); // will return true is domain is an ip.
    $staging   = array( 'staging', 'dev', 'xip', 'myftpupload', 'localhost', 'flywheelsites', 'wpengine' );
    $local     = false;

    // if domain is an IP then its local or staging
    if( $ip_loc ) {
      $local = true;
    }
    ...
    // now see if this is a staging domain.
    foreach ( $staging as $stage ) {
      if ( in_array( $stage, $parts ) ) {
        $local = true;
      }
    }
    return apply_filters( 'pl_is_local', $local );
}
 ...
 */
function pl_searchform( $echo = true, $class = 'pl-search-form', $pt = false, $placeholder = false ) {
    ...
    if( ! $placeholder ) {
      $placeholder = (pl_user_setting('search_placeholder')) ? pl_user_setting('search_placeholder') : __('Search', 'pl-platform');
    }
    ...
    $searchfield = sprintf( '<span class="btn-search"><i class="pl-icon pl-icon-search"></i></span><input type="text" value="" name="s" class="search-field" placeholder="%s" />', $placeholder );
    ...
    if ( $echo ) {
        echo apply_filters( 'pl_searchform', $searchform );
      } else {
        return apply_filters( 'pl_searchform', $searchform );
      }
}
...
function pl_pagination() {
        ...
        <ul class="pl-pager page-nav-default fix">
          <?php
                if( is_single() && ! is_page() ) {
                  ...
                  printf( '<li class="previous previous-entries">%s</li>', get_previous_posts_link( __( '&larr; Previous Entries','pl-platform' ) ) );
                  printf( '<li class="next next-entries">%s</li>', get_next_posts_link( __( 'Next Entries &rarr;','pl-platform' ) ) );
                }
                ?>
        </ul>
      <?php
  ...
  echo apply_filters( 'pl_pagination', ob_get_clean() );
}
...
function pl_nav( $args = array() ) {
    ...
    // if no menu selected, find 1st menu and use that.
    // stops users seeing a blank menu when they add a menu section.
...
function pl_create_notice( $args ) {
  ...
  // do not show messages during AJAX.
  ...
  $action = sprintf( '<span class="act">%s &nbsp; %s</span>',  $hide_btn, $action );

  if ( false === $hide ) {
    printf( '<div id="message" class="updated pl-notice %s" data-id="%s" data-exp="%s">%s <div class="pl-notice-content"><span class="ttl">%s</span> <span class="msg">%s</span>%s</div></div>',
        $args['class'],
        $id,
        $args['exp'],
        $icon,
...
/**
 * Allow themes and plugins to easily enqueue Google fonts with one single HTTP request.
 *
 * @example pl_enqueue_google_font( 'Droid Serif', array( '400', '700' ) );
 *          pl_enqueue_google_font( 'Fjalla One' );
 */
function pl_enqueue_google_font( $font = false, $args = array() ) {

  global $pl_enqueue_google_fonts;

  if( false === $font || ! is_string( $font ) ) {
    return false;
  }

  if( ! is_array( $pl_enqueue_google_fonts ) ) {
    $pl_enqueue_google_fonts = array();
  }

  /**
   * Add font to array
   */
  $pl_enqueue_google_fonts[$font] = $args;

}

/**
 * Enqueue google fonts added with pl_enqueue_google_font()
 */
function pl_enqueue_google_fonts_action() {
  ...
  if( is_array( $pl_enqueue_google_fonts ) ) {
    foreach( $pl_enqueue_google_fonts as $family => $args ) {
      if( is_array( $args ) && ! empty( $args ) ) {
        $args = ':' . implode( ',', $args );
      } else {
        $args = '';
      }
      $gfonts[] = sprintf( '%s%s', str_replace( ' ', '+', $family ), $args );
    }

    if( ! empty( $gfonts ) ) {
      $url = sprintf( '%s?family=%s', $base_url, implode( '|', $gfonts ) );
    }

    // enqueue the built font url
    wp_enqueue_style( 'pl5-google-fonts', $url  );
  }
}
add_action( 'wp_enqueue_scripts', 'pl_enqueue_google_fonts_action', 9999 );

/**
 * Enqueue components.css with optional padding css.
 */
function pl_enqueue_components() {

  pl_style( 'pl-components',  pl_framework_url( 'css' ) . '/components.css' );

  if( ! is_admin() && pl_user_setting( 'section_padding' ) ) {
    wp_add_inline_style( 'pl-components', sprintf( '.pl-sn-pad{padding: %s}', wp_strip_all_tags( pl_user_setting( 'section_padding' ) ) ) );
  }
}
