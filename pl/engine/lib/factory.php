...
class PL_Factory{
  ...
  /**
   * Load all persistent sections for the factory
   */
  function load_section_persistent() {

    // Loop through all sections
    foreach ( $this->factory as $k => $section ) {

      // if section has a required PL build, make sure we are running at least that version
      // if not, show an admin message and remove section from the factory.
      if ( version_compare( PL()->version, $section->settings['reqver'], '<' ) ) {

        // show the notice
        pl_create_notice( array(
            'title'   => 'Update Required',
            'msg'     => sprintf( 'The %s section requires at least version %s of Platform 5.',
                $section->name,
                $section->settings['reqver']
            ),
            'icon'    => 'warning',
        ));

        // remove this section from the factory so it does not show up in the builder.
        unset( $this->factory[ $k ] );
      } // load Persistent
      else {
