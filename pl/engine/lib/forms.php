...
class PL_Form_Engine {
  ...
  function set_option_up( $o ) {
    ...
    /** Cleanup strings passed via ajax callbacks */
    foreach ( $o as $index => $val ) {
      if ( 'false' == $val ) {
        $o[ $index ] = false;
      }
    ...
    if ( isset( $o['priv'] ) ) {
      if ( 'pro' == $o['priv'] && ! pl_is_professional() ) {
        $o['disabled_opt'] = 'disabled';
      }
    }
    ...
    <div class="pl-option-row pl-form-group type-<?php echo $o['type'];?> layout-<?php echo $o['layout'];?>">
      <div class="pl-option-cols">
        <div class="pl-option ">
          ...
          <label class="pl-option-title pl-input-title pl-form-label" ><?php echo $o['title']; ?> <?php echo $asterisk; ?><?php echo $this->pro_badge( $o ); ?></label>
...
<?php
  ...
  function option_multi( $o ) {
    ...
    foreach ( $o['opts'] as $key => $opt ) {
      ...
      if ( isset( $opt['title'] ) ) {
        printf( '<label class="pl-option-label">%s%s</label>', $opt['title'], $this->pro_badge( $opt ) );
      }
  ...
  function option_script( $o ) {
    ...
    <div class="script_input_wrap codemirror-wrap">
      ...
      <?php $this->setup_kses( array( 'key' => $o['key'], 'kses' => false ) ); ?>
    </div>
  ...
  function option_color( $o, $type = '' ) {
    ...
    $class = ( ! $o['disabled_opt'] ) ? 'pl-colorpicker' : '';
    ?>
    <p><input id="<?php echo $o['id'];?>" class="pl-opt <?php echo $class; ?>" type="text" name="<?php echo $o['name'];?>" placeholder="" value="<?php echo pl_color_setting( $current );?>" <?php echo $o['disabled_opt']; ?>/> <span class="pl-description"><?php echo $o['label'];?></span></p>
    <?php
  }
  ...
  function option_check( $o ) {
    ...
    <p><label for="<?php echo $o['id'];?>" class="image_uploader"><input type="hidden" class="checkbox-toggle" name="<?php echo $o['name'];?>" value="<?php echo $o['val'];?>"><input id="<?php echo $o['id'];?>" class="pl-opt checkbox-input" type="checkbox" <?php echo $val;?> <?php echo $o['disabled_opt']; ?> /> <span class="pl-description"><?php echo $o['label'];?></span></label></p>
  ...
  function option_text( $o ) {
    ?>
    <input id="<?php echo $o['id'];?>" class="pl-opt pl-form-control opt-text" type="<?php echo $o['type']; ?>" name="<?php echo $o['name'];?>" placeholder="<?php echo $o['place'];?>" value="<?php echo esc_attr( $o['val'] );?>" <?php echo $o['required'];?> <?php echo $o['disabled_opt'];?> /> <span class="pl-description"><?php echo $o['label'];?></span>
  ...
  function option_range( $o ) {
    ?>
    <input id="<?php echo $o['id'];?>" oninput="amount.value=<?php echo $o['id'];?>.value" id="<?php echo $o['id'];?>" class="pl-opt opt-text pl-range-input" type="range" name="<?php echo $o['name'];?>" min="<?php echo $o['opts']['min'];?>" max="<?php echo $o['opts']['max'];?>" step="<?php echo $o['opts']['step'];?>" value="<?php echo $o['val'];?>" <?php echo $o['required'];?> <?php echo $o['disabled_opt']; ?>/> <output class="pl-range-output" name="amount" for="<?php echo $o['id'];?>"><?php echo $o['val'];?></output><span class="pl-description"><?php echo $o['label'];?></span>
  ...
  function option_textarea( $o ) {
    ?>
    <textarea id="<?php echo $o['id'];?>" class="pl-opt pl-form-control" rows="8" type="text" name="<?php echo $o['name'];?>" placeholder="<?php echo $o['place'];?>" <?php echo $o['required'];?> <?php echo $o['disabled_opt']; ?>><?php echo $o['val'];?></textarea>
    <?php $this->setup_kses( $o ); ?>
    <?php
  }
  ...
  function option_select( $o, $type = '' ) {
    ...
    ?>
      ...
      <span class="pl-description"><?php echo $o['label'];?></span>
      <select id="<?php echo $o['id'];?>" class="pl-opt" type="select" name="<?php echo $o['name'];?>" <?php echo $o['required'];?> <?php echo $o['disabled_opt']; ?>>
  ...
  function option_image_upload( $o, $mode = 'image', $handling = 'url' ) {
    ...
    <label for="upload_image" class="image_uploader">
      ...
      <div class="image_input">
          <input id="<?php echo $o['id'];?>" class="upload_image_option pl-opt pl-form-control" type="text" size="<?php echo $size;?>" name="<?php echo $o['name'];?>" placeholder="<?php echo $place;?>" value="<?php echo $o['val'];?>" <?php echo $o['required'];?> <?php echo $o['disabled_opt']; ?>/>
          <?php if ( ! $o['disabled_opt'] ) : ?>
           <p>
  ...
  function setup_kses( $o ) {
    if ( isset( $o['kses'] ) ) {
      ...
      printf( "<input type='hidden' name='settings_kses[%s]' value='%s' />", $o['key'], wp_json_encode( $kses ) );
    }
  }
} // End of Class
...
function pl_create_form( $config ) {
  ...
  <form class="pl-standard-form" data-state="form" action="#" method="post" data-callback="<?php echo $config['callback'];?>">
    ...
    <div class="pl-form-sent pl-banner">
      <div class="pl-form-sent-message"><?php echo $config['msg_sent'];?></div>
    </div>
    <div class="pl-form">
