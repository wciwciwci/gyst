...
class PL_Platform_Settings_UI {
  ...
  function __construct( $args = array(), PL_Platform $platform ) {
    ...
    if ( $is_core_page && ! $this->platform->oauth->is_connected() && is_super_admin() && ! get_site_option( 'pl_site_registered' ) ) {
...
?>
  ...
  function platform_banner() {
    ...
    echo $this->banner( array(
        ...
        'suphead' => '',
        'header'  => 'Your PageLines Account',
        'subhead' => '<strong>Congrats!</strong> You have successfully installed PageLines. <br/>Now connect your account for extensions and updates.',
        'content' => sprintf( '<a class="pl-save-settings button button-large button-primary pl-platform-auth" href="%s">Connect &rarr;</a>', $this->platform->oauth->connect_account_link() ),
  ...
  function cards( $args = array() ) {
    ...
    $user             = get_option( '_pl_oauthed_user', '' );
    ...
    $unsubscribe_txt = 'Unsubscribe To Email Updates';
    $subscribe_txt   = 'Subscribe To Email Updates';

    if ( isset( $data->user->sub ) && 'Subscribed' == $data->user->sub ) {
      $action = 'unsubscribe';
      $txt    = $unsubscribe_txt;
      $cls    = 'updates-unsubscribe';
    } else {
      $action = 'subscribe';
      $txt    = $subscribe_txt;
      $cls    = 'updates-subscribe button-primary';
    }

    if ( isset( $data->user->user_email ) && $current_user->user_login == $user ) {
      $subscribe = sprintf( '<a data-subscribe="%s" data-unsubscribe="%s" title="%s" href="#" data-action="%s" class="extend-limited button %s" target="_blank"><span class="sub_txt"></span></a>',
          $subscribe_txt,
          $unsubscribe_txt,
          __( 'Get notifications when new extensions are released and updated.', 'pl-platform' ),
          $action,
          $cls
      );
    } else {
    ...
    <div class="pl-cards-ui" data-baseurl="<?php echo $args['baseURL'];?>">
      <div class="pl-store-head fix">
        ...
        <div class="pl-store-tag"><?php echo $subscribe; ?></div>
          ...
          <div class="pl-cards-actions">
            <div class="pl-row">
              <div class="ca-item card-actions-primary pl-col-sm-8">
                ...
                <a href="<?php echo add_query_arg( array( 'navitem' => 'extensions_multi_install' ), PL_Platform()->url() );?>" class="button button-secondary disabled extensions_multi_install extend-limited" title="Multiple Addons"></a>

                <?php if ( ! PL_Platform()->is_framework_installed() ) : ?>
                  <a class="button extend-limited" href="<?php echo PL()->operations->framework_install_url(); ?>"><i class="pl-icon pl-icon-cloud-download"></i> Install Framework</a>
                <?php endif; ?>
              </div>
