...
class PL_Platform_OAuth{
  ...
  function primary_authorization( $code, $page = 'settings' ) {
    ...
    if ( ! isset( $data->error ) && is_object( $data ) ) {
      ...
      update_option( '_pl_oauthed_user', $current_user->user_login );
  ...
  function save_cache_data( $data ) {
    update_option( $this->data_key, wp_json_encode( $data ) );
  }
  ...
  function delete_cache_data() {
    ...
    delete_option( '_pl_oauthed_user' );
  }
  ...
  function request( $endpoint = '', $args = array(), $method = 'GET', $bypass = false ) {
    ...
    if ( 'POST' == $method ) {
      $response = wp_remote_request( $this->url( $endpoint ), wp_parse_args( $args, $defaults ) );
    } else {
      $response = wp_remote_get( $this->url( $endpoint, $args ), $defaults );
    }
    ...
    if ( is_wp_error( $response ) ) {
      ...
      $result = new WP_Error( 'ssl', $error_string );
    }

    // Handle errors.
    if( ! class_exists( 'WP_Http_curl' ) ) {
      $result = new WP_Error( 'curl', __( 'It looks like you are missing the PHP cURL extension.', 'pl-platform') );
    }
    elseif( isset( $result->error ) && ( 'invalid_request' == $result->error || 'invalid_grant' == $result->error ) ) {
      $result = new WP_Error( 'oauth2', __( 'Possible Token Issue: Please try the refresh button under account, or disconnect and reconnect to get a new token.', 'pl-platform') );
    }
    elseif( isset( $result->error ) ) {
      $result = new WP_Error( $result->error, $result->error_description );
    }
  ...
  function show_error_message() {
    ...
    if ( false !== strpos( $error_string, 'SSL' ) && ! defined( 'OPENSSL_TLSEXT_SERVER_NAME' ) ) {
      $text = __( "The server you are hosted on uses a very old version of CURL and / or OpenSSL, the library used to connect to other servers.<br />Please see <a href='https://forum.pagelines.com/topic/41028-curl-error-51/'>This link</a> for more information.<br />To connect to PageLines insecurly you can add this to wp-config.php, please only do this as a last resort.<br /><kbd>define( 'PL_SSL_NO_VERIFY', true );", 'pl-platform' );
      pl_create_notice( array(
  ...
  /**
   * Get affiliate data from the main user data
   */
  function get_affiliate_data() {
    $data = $this->data();

    if ( isset( $data->user->aff ) ) {
      $aff = $data->user->aff;
    } else {
